...
class TestReviewerFailureVisibility(unittest.TestCase):
    ...
    def test_reviewer_retry_failure_reports_parse_context_without_markdown_requirement_loss(self):
        """TC6: If retry still fails, failure output includes useful parse context
        while preserving strict JSON requirement."""
        with tempfile.TemporaryDirectory() as td:
            workdir = os.path.join(td, "workdir")
            global_dir = os.path.join(td, "global")
            os.makedirs(os.path.join(workdir, ".git"))
            ...
            job_dir = os.path.join(
                global_dir, ".sdlc_runs", os.path.basename(workdir), "dummy_prd"
            )
            os.makedirs(job_dir, exist_ok=True)
            pr_file = os.path.join(job_dir, "PR_001.md")
            with open(pr_file, "w") as f:
            ...
            review_artifact = os.path.join(job_dir, "review_report.json")
            ...
            patchers = [
                patch("sys.argv", [
                    "orchestrator.py", "--workdir", workdir,
                    "--global-dir", global_dir, "--prd-file", "dummy_prd.md",
                    "--force-replan", "false", "--enable-exec-from-workspace",
                    "--channel", "test-channel", "--max-prs-to-process", "1",
                    "--engine", "gemini",
                ]),
                patch("orchestrator.load_engine_registry", return_value={
                    "engines": {
                        "gemini_direct_cli": {
                ...
                patch("orchestrator.get_mainline_branch", return_value="master"),
                patch("orchestrator.get_head_commit_hash", return_value="deadbeef"),
                patch("orchestrator.classify_coder_null_output",
                      return_value=(False, "", "deadbeef")),
                patch("agent_driver.send_ignition_handshake"),
                patch("orchestrator.get_env_with_gemini_key",
            ...
            self.assertIn(
                ...
                f"Expected 'reviewer_invalid_json' in {notified_event_types}",
            )
            self.assertNotIn(
                "reviewer_no_output",
                notified_event_types,
                "Stateless path should use inline-alert, not report no_output",
            )

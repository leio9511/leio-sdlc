...
class TestAgentDriverTriad(unittest.TestCase):
    ...
    def test_pm_payload_injection(self, mock_invoke_agent):
        ...
        prd_file = os.path.join(self.workdir, "PRD.md")
        context_file = os.path.join(self.workdir, "context.md")
        with open(prd_file, "w") as f:
            f.write("mock_prd_content_for_pm")
        with open(context_file, "w") as f:
            f.write("mock_context_content_for_pm")
            
        test_args = ["pm.py", "--prd-file", prd_file, "--context-file", context_file, "--workdir", self.workdir]

...
# PRD: Gemini CLI Native Deployment for leio-sdlc
...
- **FR-6**: 配合 Gemini CLI Policy Engine 的 blacklist 机制，在 `--yolo` 模式下仍然拦截高危命令（如 `rm -rf /`、`dd`、`mkfs` 等），确保 AI 幻觉无法执行毁灭性操作。
...
### 3.3 安全层：Policy Engine Blacklist（配合 yolo 使用）

**重要设计决策：不使用 `--sandbox`（Boss 确认）**
因为代码仓库在宿主机上，sandbox 容器无法访问工作区文件，所以采用 Policy Engine blacklist 替代沙盒隔离。

**Policy Engine 决策链**：
```
命令进来
    ↓
检查 deny 列表 → 发现 rm -rf / → 直接拦截（优先级高于 yolo）
    ↓
不在 deny 列表 → yolo 模式自动放行（无弹窗）
```

**用户需在目标机器配置 `~/.gemini/policies/sdlc.toml`**：
```toml
# 允许安全命令自动执行
[[rule]]
...
**为什么 yolo + blacklist 比 sandbox 更适合**：
- yolo 确保流水线不挂起
- blacklist 在命令层拦截高危操作（yolo 无法覆盖 deny 规则）
- 工作区文件保持可访问（无需容器挂载）
...
### 3.4 技能层：SKILL.md 兼容性（向下兼容 OpenClaw）
...
- `~/.gemini/policies/sdlc.toml` — **新增**（用户创建），Gemini CLI Policy Engine blacklist 配置
...
### ~/.gemini/policies/sdlc.toml（用户需在目标机器创建）：
```toml
...
# Block dangerous commands even in --yolo mode
[[rule]]
toolName = "run_shell_command"
commandPrefix = "rm -rf /"
decision = "deny"

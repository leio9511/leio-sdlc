...
ALLOWED_AMBIENT_SNIPPETS = (
    # scripts/dev_python.sh owns repo-venv creation; target scripts must call that wrapper.
...
def test_guardrail_bash_targets_do_not_use_ambient_python_for_contract_critical_calls():
    offenders: list[str] = []
    for rel_path in GUARDRAIL_BASH_TARGETS:
...
def test_guardrail_bash_targets_reference_dev_python_wrapper():
    missing = [
        rel_path
        for rel_path in GUARDRAIL_BASH_TARGETS
...
def test_guardrail_bash_targets_removed_from_trap_manifest():
    manifest = json.loads((REPO_ROOT / "ignore_tests.json").read_text(encoding="utf-8"))
    bash_ignored = set(manifest.get("bash", []))
    still_ignored = sorted(set(GUARDRAIL_BASH_TARGETS) & bash_ignored)

    assert not still_ignored, "Guardrail bash targets still ignored: " + ", ".join(still_ignored)


def test_guardrail_bash_targets_pass_with_clean_trap_ambient_python(tmp_path: Path):

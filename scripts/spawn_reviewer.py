...
def main():
    ...
    task_string = (
        ...
        f"\n"
        f"[STRICT CONTEXT ISOLATION]\n"
        f"You MUST cleanly isolate `recent_history.diff` from `current_review.diff`.\n"
        f"`recent_history.diff` is strictly read-only reference material to check if requirements were previously satisfied.\n"
        f"All security checks, redlines, and logic validations MUST be strictly applied ONLY to `current_review.diff`.\n"
        f"DO NOT reject the current PR based on code found in `recent_history.diff`.\n"
        f"Judge the current Coder ONLY on what is in `current_review.diff`.\n"
        f"\n"
        f"[EXEMPTION CLAUSE]\n"

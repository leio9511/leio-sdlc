...
class TestSpawnVerifier(unittest.TestCase):
    @patch('spawn_verifier.save_envelope_artifacts')
    @patch('spawn_verifier.invoke_agent')
    @patch('utils_api_key.setup_spawner_api_key')
    @patch('os.path.exists')
    @patch('os.chdir')
    @patch('os.makedirs')
    def test_spawn_verifier_payload_injection(self, mock_makedirs, mock_chdir, mock_exists, mock_setup_key, mock_invoke_agent, mock_save_artifacts):
    ...
    @patch('os.chdir')
    @patch('os.makedirs')
    def test_verifier_aligned_to_file_based_result(self, mock_makedirs, mock_chdir, mock_exists, mock_setup_key, mock_invoke_agent, mock_save_artifacts):
        import spawn_verifier
        from agent_driver import AgentResult

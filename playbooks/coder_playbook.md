...
3. **Commit**:
   ...
   - When committing code through the official coder path, use the shared runtime helper with the coder role through the controlled development entrypoint instead of a raw privileged commit command: `./scripts/dev_python.sh scripts/runtime_git_identity.py --role coder -- commit -m "feat/fix: <description>"`
   - **MANDATORY EXIT CRITERIA:** You MUST meet all three conditions before finishing your turn:
     ...
     c. Your Git status is absolutely clean. You MUST explicitly execute `./scripts/dev_python.sh scripts/runtime_git_identity.py --role coder -- commit -m "feat/fix: <description>"` to commit your staged files. Uncommitted changes will be rejected by the Orchestrator.

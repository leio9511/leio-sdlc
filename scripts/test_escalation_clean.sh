...
SDLC_BYPASS_BRANCH_CHECK=1 python3 scripts/orchestrator.py --channel "#test" --channel "#test" --workdir "$(pwd)" --global-dir "$PROJECT_ROOT" --prd-file dummy_prd.md --max-prs-to-process 2 --coder-session-strategy always > orchestrator.log 2>&1 || true

...
def mock_workdir_with_preflight():
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        Path(os.path.join(temp_dir, 'preflight.sh')).write_text('#!/bin/bash\necho preflight\n', encoding='utf-8')
        yield temp_dir
...
def _prepare_job_dir(workdir):
    ...
    job_dir = os.path.join(workdir, '.sdlc_runs', target_project_name, 'dummy_prd')
    os.makedirs(job_dir, exist_ok=True)
    pr_file = os.path.join(workdir, 'PR_001_test.md')
    Path(pr_file).write_text('status: in_progress\n', encoding='utf-8')
    return job_dir, pr_file
...
def test_null_output_routes_back_to_coder_system_alert_before_reviewer(mock_workdir_with_preflight):
    ...
    _job_dir, pr_file = _prepare_job_dir(mock_workdir)
    ...
    with patch('orchestrator.SanityContext.perform_healthy_check'), \
        ...
        def dummy_drun(cmd, *args, **kwargs):
            if isinstance(cmd, list):
                cmd_str = ' '.join(cmd)
                if cmd[:4] == ['git', 'rev-parse', 'HEAD']:
                    return _mock_result('abc123\n')
...
def test_repeated_null_output_exhausts_yellow_budget_and_enters_existing_red_path(mock_workdir_with_preflight):
    ...
    Path(os.path.join(config_dir, 'sdlc_config.json')).write_text('{"YELLOW_RETRY_LIMIT": 2, "RED_RETRY_LIMIT": 1}', encoding='utf-8')

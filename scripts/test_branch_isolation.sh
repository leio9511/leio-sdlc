...
git stash push -m "temp_test_stash" && git checkout master || git checkout main

export SDLC_TEST_MODE=true

set +e
OUTPUT=$(python3 scripts/spawn_coder.py --workdir "$WORKSPACE" --global-dir "$WORKSPACE" --pr-file dummy_pr.md --prd-file dummy_prd.md 2>&1)
EXIT_CODE=$?
set -e
...
if ! echo "$OUTPUT" | grep -q '"role": "coder"'; then

...
print_help() {
    ...
    echo "  Formal Python-backed development/test checks must use controlled entries:"
    echo "    bash scripts/dev_python.sh -m pytest ..."
    echo "    bash preflight.sh --report-all"
    echo "  Do not rely on manual activation or ambient Python for formal checks."
    echo ""
    echo "Options:"

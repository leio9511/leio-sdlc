...
scripts/dev_python.sh scripts/orchestrator.py --enable-exec-from-workspace --force-replan true --enable-exec-from-workspace --channel "valid:id" --workdir "$(pwd)" --global-dir "$MOCK_GLOBAL_DIR" --prd-file dummy_prd.md --max-prs-to-process 1 --coder-session-strategy always > orchestrator.log 2>&1 || true
...
scripts/dev_python.sh scripts/orchestrator.py --enable-exec-from-workspace --force-replan true --cleanup --workdir "$(pwd)" --global-dir "$MOCK_GLOBAL_DIR" --prd-file dummy_prd.md > cleanup.log 2>&1 || true

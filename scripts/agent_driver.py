...
def is_case1_eligible(engine: str) -> bool:
    """Check if an engine satisfies the case-1 definition.

    A case-1 engine must support authoritative bootstrap-time resume-id
    acquisition and reliable resume by that identifier.

    - OpenClaw (`engine == "openclaw"`) is always case-1 eligible (FR-4)
    - Gemini (`engine == "gemini"`) eligibility must be determined by the
      bootstrap artifact result — it is NOT presumed eligible
    - Any engine not explicitly classified as case-1 is NOT eligible
    """
    if not engine:
        return False
    engine = engine.lower()
...
def invoke_agent_gated(
    ...
    - ``case1_strict`` mode:
      - OpenClaw engine: delegate to ``invoke_agent()`` (baseline, no two-phase)
      - Gemini engine: delegate to ``invoke_agent_two_phase()``
    ...
    # ── case1_strict mode ───────────────────────────────────────────
    # OpenClaw is the case-1 baseline — direct invoke, no two-phase (FR-4)
    if engine == "openclaw":
        return invoke_agent(
            task_string,
            session_key=session_key,
    ...
    # Gemini uses two-phase bootstrap/continue protocol (PR-003)
    if engine == "gemini":
        return invoke_agent_two_phase(
            task_string,
            session_key=session_key,
            role=role,
            run_dir=run_dir,
...
def assert_no_heuristic_downgrade(mode: str, fallback_attempted: bool) -> None:
    """Safety net: raise RuntimeError if heuristic fallback is attempted in case1_strict mode.

    This guard provides defense-in-depth against future regressions that
    might try to silently downgrade from case1_strict to heuristic discovery.
    It should never be triggered in normal operation.
    """
    if fallback_attempted and mode == "case1_strict":
        raise RuntimeError(
            "Heuristic downgrade detected in case1_strict mode. "

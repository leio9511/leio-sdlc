...
7. 若真实 Gemini CLI 受控验证需要额外 launch profile（例如适合 SDLC 的自动批准 / YOLO 模式），该 launch profile 属于 engine-specific runtime policy，而非 ACP protocol 通用能力；其是否启用与如何启用必须在实现中作为显式、可审计的启动配置表达，而不得混入 #49 的核心 contract 语义。
...
- 若 Gemini CLI 的 ACP 路径需要连接配置、endpoint 标识、会话发现方式或启动前提,这些内容必须在实现中通过清晰、可审计的配置或常量入口表达,禁止把目标对象隐藏在临时脚本、一次性命令行手工参数或不可追踪的机器局部约定中。Gemini CLI (v0.40.1) 应当被启动为一个子进程并带上 `--acp` 参数,测试与 validator 的协议交互默认通过标准的 Stdio(标准输入输出管道)管道模式进行,这是本地最确定的无端口 IPC 通信方式。

默认文件落点必须如下：
- 运行时实现放在 `scripts/` 下
- 自动化测试放在 `tests/` 下
- 测试夹具与结构化样例放在 `tests/fixtures/` 下（若需要新增）
- 真实 Gemini CLI 运行所得的结构化 verdict / observation 产物必须落到 `.sdlc_runs/acp/<target-cli>/` 下，并至少支持一个稳定的 `latest.json` 指针与一个 timestamped artifact，例如 `.sdlc_runs/acp/gemini/latest.json` 与 `.sdlc_runs/acp/gemini/<timestamp>.json`。这些运行时 artifact 默认不作为仓库固定样例纳入 git。
- 测试静态 Mock 骨架、Schema 定义与固定 sample contract 放在 `tests/fixtures/` 下，例如 `tests/fixtures/acp_verdict_schema.json` 与 `tests/fixtures/acp_verdict_gemini_sample.json`。禁止任何散落到临时目录或随意分配位置的行为。
...
其中职责边界必须明确：
- `scripts/acp_probe.py` 负责最小 probe 逻辑与 contract observation 映射，至少承载 `connect / execute_turn / capture_handle / resume_once / classify_failure / emit_verdict` 的核心语义。
- `scripts/acp_smoke.py` 负责作为受控 smoke / exploratory runner，拉起目标 CLI（默认 Gemini CLI）并驱动 `acp_probe.py`，同时负责将结构化 verdict 落盘到规定 artifact 路径。
- `scripts/acp_client.py` 负责对 ACP Python SDK 的最小封装，不得在本 issue 内膨胀为通用 runtime manager。
...
- 真实环境探测时，如果宿主环境缺少 `GEMINI_API_KEY` 环境变量，validator 必须能够优雅捕获此状态，并在 `failure_classification` 中归类为 “Missing API Key”，在 `final_verdict` 中填入 `blocked` 或 `partially_supported` 并正常结束，绝对禁止抛出未捕获的 Traceback 或直接导致 Preflight 阻塞性报错。
- baseline validation 默认针对最小 ACP lane 与 contract-relevant behavior；若要模拟更接近 SDLC 的自动批准 / YOLO 执行策略，应将其放入 exploratory complex smoke，并明确标记为 engine-specific launch profile observation，而不是把 YOLO 语义提升为 ACP protocol-native capability 或 #49 核心 contract 字段。
...
- artifact 必须至少包含以下字段:
  ...
  - `capability_surface`
  - `final_verdict`
- artifact 的生成方式与固定落点必须在实现中清晰、可审计；真实 Gemini CLI 运行产物默认落在 `.sdlc_runs/acp/<target-cli>/`，而纳入仓库的只有 schema / sample contract / fixture，不得把环境相关真实 verdict 直接固化到 repo 配置目录中
...
- **v1.7**: 明确 Gemini CLI baseline connection surface 为 subprocess + `--acp` + stdio；补充 `GEMINI_API_KEY` 缺失时的优雅 verdict 归类要求；将真实 verdict artifact 从 repo 固定配置路径调整为 `.sdlc_runs/acp/<target-cli>/` 运行产物路径，并明确仓库内仅保留 fixture/schema/sample contract；同时明确 `acp_probe.py` / `acp_smoke.py` / `acp_client.py` 的职责边界，并澄清 YOLO/auto-approve 属于 engine-specific launch profile，而非 ACP protocol-native contract。
...
- **`real_verdict_runtime_artifact_layout`**:
```text
.sdlc_runs/acp/gemini/latest.json
.sdlc_runs/acp/gemini/<timestamp>.json
```

- **`repo_fixture_and_sample_contract_files`**:
```text
tests/fixtures/acp_verdict_schema.json
tests/fixtures/acp_verdict_gemini_sample.json
```

- **`acp_probe_responsibility_note`**:
```text
acp_probe.py owns minimal ACP probe logic and contract-relevant observation mapping.
```

- **`acp_smoke_responsibility_note`**:
```text
acp_smoke.py owns the controlled smoke/exploratory runner for Gemini CLI and writes structured verdict artifacts.
```

- **`acp_yolo_policy_note`**:
```text
YOLO / auto-approve is an engine-specific launch profile, not an ACP protocol-native capability and not a core #49 contract field.
```

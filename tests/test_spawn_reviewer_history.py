...
def test_spawn_reviewer_uses_baseline_if_present():
    with tempfile.TemporaryDirectory() as td:
        ...
        with patch("sys.argv", test_args), \
             ...
             patch("spawn_reviewer.invoke_agent_gated", return_value=AgentResult(session_key='...', stdout='{"status":"APPROVED"}', stderr='', return_code=0)), \
...
def test_spawn_reviewer_creates_session_file():
    with tempfile.TemporaryDirectory() as td:
        ...
        with patch("sys.argv", test_args), \
             ...
             patch("spawn_reviewer.check_guardrails", return_value=None), \
             patch("spawn_reviewer.invoke_agent_gated", return_value=AgentResult(session_key='subtask-123', stdout='{"status":"APPROVED"}', stderr='', return_code=0)), \

...
class TestDeployExcludes(unittest.TestCase):
    ...
    def test_deploy_excludes_tests(self):
        with isolated_repo_env(self.project_root) as isolated:
            ...
            res = subprocess.run(["bash", deploy_script, "--no-restart"], env=env, cwd=repo_root, capture_output=True, text=True)
    ...
    def test_deploy_excludes_sdlc_runs(self):
        with isolated_repo_env(self.project_root) as isolated:
            ...
            isolated_sdlc = os.path.join(repo_root, ".sdlc")
            isolated_runs = os.path.join(repo_root, ".sdlc_runs")
            os.makedirs(isolated_sdlc, exist_ok=True)
            os.makedirs(isolated_runs, exist_ok=True)

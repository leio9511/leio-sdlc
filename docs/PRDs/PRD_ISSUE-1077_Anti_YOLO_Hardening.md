...
-   **`templates/PRD.md.template`**: Add a `## 7. HARDCODED CONTENT (CODER MUST COPY EXACTLY)` section.
...
-   **Scenario 1: Auditor Reject Flow with JIT**
    ...
    -   **Then** Slack receives status updates and `stdout` ends with the rejection JIT prompt.
...
**1. Content for `templates/PRD.md.template`:**

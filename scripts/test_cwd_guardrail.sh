...
if python3 scripts/orchestrator.py --enable-exec-from-workspace --force-replan true --enable-exec-from-workspace --enable-exec-from-workspace --workdir "$INNER_DIR" --prd-file "prd.md" --channel "valid:id" --global-dir "$PROJECT_ROOT" 2>&1 | grep -q "Git boundary violation"; then

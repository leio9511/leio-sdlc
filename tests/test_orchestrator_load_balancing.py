...
def test_no_keys_configured():
    ...
    try:
        ...
        # Or missing entirely
        res = assign_gemini_api_key("session_123", None, state_file_path)
        assert res is None
    finally:
...
def test_first_assignment_persists():
    ...
    with tempfile.NamedTemporaryFile(delete=False) as tf:
        state_file_path = tf.name
    try:
        res = assign_gemini_api_key("session_123", keys, state_file_path)
        assert res in keys
...
def test_anti_drift_stickiness():
    ...
    with tempfile.NamedTemporaryFile(delete=False) as tf:
        state_file_path = tf.name
    try:
        res1 = assign_gemini_api_key("session_xyz", keys_initial, state_file_path)
        assert res1 in keys_initial
        
        # Modify the order
        keys_modified = ["key_C_11112222", "key_A_12345678", "key_B_abcdefgh"]
        res2 = assign_gemini_api_key("session_xyz", keys_modified, state_file_path)
...
def test_graceful_degradation():
    ...
    try:
        ...
        res2 = assign_gemini_api_key("session_xyz", keys_missing, state_file_path)
